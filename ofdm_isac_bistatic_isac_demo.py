...
Status:
  ...
  Phase 1D — IMPLEMENTED  : Real-time dashboard 6-panel (--plot): comm + JCAS + range
  Phase 1E — DONE         : Lab validation static vs walk (kontras amp_hp ~32×)
  Phase 1F — IMPLEMENTED  : Forward-scatter JCAS detector (amp/phase LoS, MAD-CFAR)
                            = KLAIM DETEKSI utama. Tuning final: k=3.5, min_score=1.2.
                            Range/echo = konteks visual saja (bukan klaim).
...
Usage:
  # ISAC demo (comm + JCAS sensing) — mode utama, tuning final
  python3 ofdm_isac_bistatic_isac_demo.py --phase1b --plot --fs 40e6 --frames 3000 \
      ...
      --log-csv run.csv
  # Angka resmi (tanpa plot, floor false-alarm bersih)
  python3 ofdm_isac_bistatic_isac_demo.py --phase1b --fs 40e6 --frames 1500 \
      --tx-gain 90 --rx-gain 76 --sense-threshold-k 3.5 --sense-min-score 1.2 \
      --log-csv static.csv
  # Matikan detektor JCAS (comm saja)
  python3 ofdm_isac_bistatic_isac_demo.py --phase1b --fs 40e6 --frames 500 --no-jcas
  # FS sweep (tanpa phase1b)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="ofdm_isac_bistatic_isac_demo — FS sweep / V2I comm / "
                    "JCAS forward-scatter object detection (ISAC)")
    ...
    ap.add_argument("--log-csv", type=str, default=None,
                    help="Per-frame ISAC log CSV (comm + range + JCAS), Phase 1B. "
                         "Auto-nama isac_log_<ts>.csv bila --phase1b tanpa --log-csv.")
